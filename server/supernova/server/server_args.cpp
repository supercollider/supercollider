...
server_arguments::server_arguments(int argc, char* argv[])
{
    ...
    options.add_options()("help,h",
                          "show this help")("udp-port,u", value<uint32_t>(&udp_port)->default_value(0), "udp port")(
        ...
        "use-system-clock,C",
        value<uint16_t>(&use_system_clock)->default_value(0),
        ...
        "verbose,V",
        value<int16_t>(&verbosity)->default_value(0),
...
#ifdef _WIN32
        ("ugen-search-path,U",
         value<vector<string>>(&ugen_paths),
...
#endif
            ("restricted-path,P",
             value<vector<string>>(&restrict_paths),
             "if specified, prevents file-accessing OSC commands from accessing files outside <restricted-path>")(
                "threads,T",
                value<uint16_t>(&threads)->default_value(boost::thread::physical_concurrency()),
