...
bool initAwakeMessage(VMGlobals *g)
{
	...
	g->process->curThread = g->process->mainThread; //??
	g->thread = g->process->mainThread.uot; //??
	...
	g->thread->beats = slot[1];
	g->thread->seconds = slot[2];
	g->thread->clock = slot[3];
...
bool initInterpreter(VMGlobals *g, PyrSymbol *selector, int numArgsPushed)
{
	...
	g->thread->beats.utag = tagFloat;
	g->thread->seconds.utag = tagFloat;
...
void StoreToImmutableA(VMGlobals *g, PyrSlot *& sp, unsigned char *& ip)
{
	...
	sp[1] = sp[0]; // copy value up one
	sp[0] = g->receiver; // put receiver in place
	sp++;
	g->sp = sp;
	g->ip = ip;
...
//	do_check_pool(pyr_pool_compile);
	...
	switch (op1) {
		...
		case 4 : // opExtended, opPushLiteral
			...
			if (slot->utag == tagObj && slot->uo->classptr == gSpecialClasses[op_class_fundef]->u.classobj) {
				...
				if (IsNil(&slot->uoblk->contextDef)) {
					closure->context = g->process->interpreter.uoi->context;
				} else {
		...
		case 5 : // opExtended, opPushClassVar
			...
			ip += 2;
			*++sp = g->classvars->slots[(op2<<8)|op3];
			break;
		...
		case 9 : // opExtended, opStoreClassVar
			...
			op3 = ip[2]; // get class var index
			ip += 2;
			g->classvars->slots[(op2<<8)|op3] = *sp;
		...
		case 15 : // opExtended, opSpecialOpcode (none yet)
			...
			break;
		// opPushInstVar, 0..15
		case 16 : *++sp = g->receiver.uo->slots[ 0]; break;
		case 17 : *++sp = g->receiver.uo->slots[ 1]; break;
		case 18 : *++sp = g->receiver.uo->slots[ 2]; break;
		case 19 : *++sp = g->receiver.uo->slots[ 3]; break;
		case 20 : *++sp = g->receiver.uo->slots[ 4]; break;
		case 21 : *++sp = g->receiver.uo->slots[ 5]; break;
		case 22 : *++sp = g->receiver.uo->slots[ 6]; break;
		case 23 : *++sp = g->receiver.uo->slots[ 7]; break;
		case 24 : *++sp = g->receiver.uo->slots[ 8]; break;
		case 25 : *++sp = g->receiver.uo->slots[ 9]; break;
		case 26 : *++sp = g->receiver.uo->slots[10]; break;
		case 27 : *++sp = g->receiver.uo->slots[11]; break;
		case 28 : *++sp = g->receiver.uo->slots[12]; break;
		case 29 : *++sp = g->receiver.uo->slots[13]; break;
		case 30 : *++sp = g->receiver.uo->slots[14]; break;
		case 31 : *++sp = g->receiver.uo->slots[15]; break;
		...
		case 33 : *++sp = g->frame->context.uof->vars[ip[1]]; ip++; break;
		case 34 : *++sp = g->frame->context.uof->context.uof->vars[ip[1]]; ip++; break;
		case 35 : *++sp = g->frame->context.uof->context.uof->context.uof->vars[ip[1]]; ip++; break;
		...
		case 40 : 
			ival = ip[1];
			ip+=1;
			*++sp = g->block->constants.uo->slots[ival];
			break;
		case 41 : 
			ival = (ip[1] << 8) | ip[2];
			ip+=2;
		...
		// opPushTempZeroVar
		case 48 : *++sp = g->frame->vars[ 0]; break;
		case 49 : *++sp = g->frame->vars[ 1]; break;
		case 50 : *++sp = g->frame->vars[ 2]; break;
		case 51 : *++sp = g->frame->vars[ 3]; break;
		case 52 : *++sp = g->frame->vars[ 4]; break;
		case 53 : *++sp = g->frame->vars[ 5]; break;
		case 54 : *++sp = g->frame->vars[ 6]; break;
		case 55 : *++sp = g->frame->vars[ 7]; break;
		case 56 : *++sp = g->frame->vars[ 8]; break;
		case 57 : *++sp = g->frame->vars[ 9]; break;
		case 58 : *++sp = g->frame->vars[10]; break;
		case 59 : *++sp = g->frame->vars[11]; break;
		case 60 : *++sp = g->frame->vars[12]; break;
		case 61 : *++sp = g->frame->vars[13]; break;
		case 62 : *++sp = g->frame->vars[14]; break;
		case 63 : *++sp = g->frame->vars[15]; break;
		
		// case opPushLiteral
		case 64 : *++sp = g->block->constants.uo->slots[ 0]; break;
		case 65 : *++sp = g->block->constants.uo->slots[ 1]; break;
		case 66 : *++sp = g->block->constants.uo->slots[ 2]; break;
		case 67 : *++sp = g->block->constants.uo->slots[ 3]; break;
		case 68 : *++sp = g->block->constants.uo->slots[ 4]; break;
		case 69 : *++sp = g->block->constants.uo->slots[ 5]; break;
		case 70 : *++sp = g->block->constants.uo->slots[ 6]; break;
		case 71 : *++sp = g->block->constants.uo->slots[ 7]; break;
		case 72 : *++sp = g->block->constants.uo->slots[ 8]; break;
		case 73 : *++sp = g->block->constants.uo->slots[ 9]; break;
		case 74 : *++sp = g->block->constants.uo->slots[10]; break;
		case 75 : *++sp = g->block->constants.uo->slots[11]; break;
		case 76 : *++sp = g->block->constants.uo->slots[12]; break;
		case 77 : *++sp = g->block->constants.uo->slots[13]; break;
		case 78 : *++sp = g->block->constants.uo->slots[14]; break;
		case 79 : *++sp = g->block->constants.uo->slots[15]; break;
			...
			if (meth->name.us != selector) {
				...
				switch (methraw->methType) {
					...
					case methReturnLiteral : /* return literal */
						sp -= numArgsPushed - 1;
						*sp = meth->selectors; /* in this case selectors is just a single value */
						break;
					...
					case methAssignInstVar : /* assign inst var */
						...
						if (obj->obj_flags & obj_immutable) { StoreToImmutableB(g, (PyrSlot*&)sp, ip); }
							...
							if (numArgsPushed >= 2) {
								obj->slots[index] = sp[1];
								g->gc->GCWrite(obj, sp + 1);
							} else {
								obj->slots[index] = gSpecialValues[svNil];
							}
							*sp = *slot;
						}
						break;
					case methReturnClassVar : /* return class var */
						sp -= numArgsPushed - 1;
						*sp = g->classvars->slots[methraw->specialIndex];
						break;
					case methAssignClassVar : /* assign class var */
						sp -= numArgsPushed - 1;
						if (numArgsPushed >= 2) {
							g->classvars->slots[methraw->specialIndex] = sp[1];
							g->gc->GCWrite(g->classvars, sp + 1);
						} else {
							g->classvars->slots[methraw->specialIndex] = gSpecialValues[svNil];
						}
						*sp = *slot;
						break;
					...
					case methForwardClassVar : /* forward to an instance variable */
						...
						selector = meth->selectors.us;
						*slot = g->classvars->slots[methraw->specialIndex];				
...
bool BadStackTags(VMGlobals *g)
{
	...
	PyrSlot *slot =  g->sp;
	for (; slot>=bottom; slot--) {
		if (slot->tag < 1 || slot->tag > 9) return true;
	}
	return false;
}
