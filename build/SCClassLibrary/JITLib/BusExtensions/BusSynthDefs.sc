...
*initClass {
	...
	defs = [
		...
		SynthDef("system_line_control", { arg i_bus=0, i_start=0.0, i_end=1.0, i_dur=1.0;
			ReplaceOut.kr(i_bus, Line.kr(i_start, i_end, i_dur, doneAction:2))
		}),

		SynthDef("system_xline_control", { arg i_bus=0, i_start=0.001, i_end=1.0, i_dur=1.0;
			ReplaceOut.kr(i_bus, XLine.kr(i_start, i_end, i_dur, doneAction:2))
		}),
