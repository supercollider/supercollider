...
SC has life in social media too. Visit a lively community at sccode.org:
...
@sc140tweets. Also, you are strongly encouraged to subscribe to the two SC
...
alternative install path when prompted during installation. To
...
IMPORTANT: To use the Quarks extension system you *need* to install
...
be one that is supported by [PortAudio][portaudio]. For professionals
...
can choose. Usually, you get the best results and biggest control over latency by
...
To start up the IDE use the start menu link or runs the scide.exe program found
within the installation folder. On the first start-up expect several firewall
...
`userAppSupportDir` (soft linking is not supported anymore).
...
the version of each library as appropriate.
...
Note that a slightly patched version of the PortAudio source is provided as
a submodule. PortAudio can support up to five different backends: MME, DSound,
...
set the working directory there to allow sclang to find the server (for more
information on using QtC see the walkthrough below).

If you want to run SuperCollider independently of the built environment:
...
Note that for just passing SC to a friend it is not necessary to create
...
always lose the local customizations and additions stored in the
...
learn about the options that the builder provides. With default settings, all APIs
that *work out of the box* is enabled and only the library for static linking
...
fix any build problems. Switching git branches that only contain changes to
...
get an error message at SC start-up. This is a good thing because it
allows identifying the actual problem. Far worse are generic error messages or
...
build to work outside of the built environment (*standalone* in the meaning of
...
significance of environment settings when using CMake-Gui, please refer to
...
be used on the reconfiguration of the build. This is equivalent to passing in
...
`-D`. During the build, cache values are prioritized overvalues any cmake
...
CMakeCache.txt. So keep in mind that implementing a different logic to create value for a cmake variable (or e.g. moving a library to a place where it will be
...
to use that value. A safe fix for this kind of problem is to delete the entire
...
folders - using and depending on the built environment settings. The development
...
On Windows, the bundled utility just reads out the dynamic libraries required to
run an executable, tries to find them in the system, and copies them to the folder in which the executable resides. This is the safest way to guarantee that
SC can be used independently of the built environment because Windows
...
the executable folder. Furthermore, it is the last install step, you just need
to scroll up the cmake build output. So if you get obscure Windows system
messages at SC startup, try to scroll up the install output, and go over the
...
Windows yet. Therefore you are strongly encouraged to report Windows to build
...
Initialization and population of the submodules were automatically done because
...
To find out which CMake variables need values, we can do a trial run.
For this to be meaningful we need to
  ...
  - point `cmake` to the source (the last element in `cmake` command, cannot be
...
for the alternative audio server `supernova` is available (not available in
...
and contains space, the install fails. Furthermore, installing the
...
To correct this error, we need to rerun the configuration and set an
install folder in userspace that does not contain a space. The variable for
...
the second build should proceed very quickly as CMake only builds changed components
from scratch.

If you execute a build command, CMake compares the timestamps of source files
with the timestamp of the last build, to determine if anything has changed. So
if you don't need to verify that reconfiguration was successful, you can just
...
must be set on the system level. The same is true for a development IDE.
...
is an important caveat: when running from the Windows Start Menu, CMake-Gui runs in
...
path adjustments on the system level (using Control Panel/System/Advanced System
...
So we need to keep in mind: While working on an SC build with changed *system
...
1. the Qt location
  - While Qt can be added via the PATH as well, it is safer to define an
    environment variable "CMAKE_PREFIX_PATH", that contains the path to the parent folder of the "Qt-system" we are using. The Qt-architecture
    ...
    a binaries search path, but an anchor path for CMake to work out all relevant Qt locations.
    ...
    Applications behave differently as to whether they instantly react to system
    the environment changes or sticks to the environment set up at login or system
    boot. So far it looks as if cmake-gui, Visual Studio and also the file-
    explorer is among the applications that sense environment changes
...
configure the build. If this attempt returns errors, you will have to provide
additional values. After the configuration CMake-Gui will present you with a
long list of cmake variables alongside the values assigned to them as
stored in the cache. In the box at the bottom you can scroll through the output
CMake produced during configuration and analyze it. At this stage, it is good to
...
theory only NOT-FOUNDs *creating a CMake error* should be a problem.
...
If you set the environment on the Windows system level, you can start Visual Studio
...
To boot the server from scide or sclang, it is necessary to set the
working directory for the respective target. This can be done in the "Debug"
the pane of the target properties dialog.

*Important Note*: provided the built environment is set up properly (the main
...
folders. If you want to run SC outside of the built environment or copy the
build to a different computer, you need to build the target "install". This step
will create an autonomous install of SuperCollider, without any outside
...
separate install folder, and a CMake utility, "bundle utilities", try to
...
executables directly from within QtC. To start the server from the
...
the build similarly using CMake as CMake-Gui does. You can also do the trial
...
that QtC already created a kit at a start-up that fits the needs. While QtC can be
...
You can also choose individual targets to be built here. Each build step can be
...
To create a binary installer, you will want to add the (custom) target
...
* Location where SC gets "installed", which in this context means: where the
  SC files - including required external libraries - get copied to if you build the target "install". The default location is set to './Install'.
  If you maintain multiple builds you could either use different build trees or install to different folders. Change this, for example, like so:

      -D CMAKE_INSTALL_PREFIX=./x86/AllPullRequests

  This installs the files to a subfolder x86/AllPullRequests of the build-
  directory. Note that CMake uses forward slash as folder separator on all platforms. You may also specify an absolute path here. Building the install target is a precondition for creating a binary installer. Apart from copying
  ...
  then the system libraries guaranteed to be available on every Windows system.
...
* Build the *supernova* server:
  ...
  *Note*: When you build with supernova, an alternative server executable and a supernova version of each plugin is built. If you also use the 'sc3-plugins'
  package, make sure to compile them with supernova support too. At the time of this writing, supernova could only be built with the MinGW toolchain.

  Within SC you will be able to switch between scsynth and supernova by
  evaluating one of the lines below and then booting the server as usual:
  ...
  *Note*: If you experience problems getting the supernova plugins recognized
  ...
  "plugins" in the "userAppSupportDirectory\Extensions" folder. It may also be required to separate binary plugins and classes. The class files may reside
  ...
  *Note*: While there is no readily available 64-bit Qt version for the MinGW
  ...
  64-bit build of supernova currently breaks but might be more easy to fix than the VS build.
...
* It is possible to build SuperCollider without the IDE, and even without Qt
  ...
  SC will not link to Qt at all and implicitly build without the IDE. This build variation is used in low resources environments and graphic-less use scenarios of SC. In the Qt-less build some Qt/GUI related classes are
  ...
  SCClassLibrary/scide_scqt. If you need to run code with elements considered deprecated in SC3.7, you can move the deprecated/3.7 folder back into place.
  You should remove the unsupported Document-class related entries to avoid
  (non-fatal) errors at sclang start-up.

  You cannot use the Help browser in Qt-less SC. An option is, to render Help with sclang (`SCDoc.renderAll`) and run it in a browser. There is also the
...
* Server-only: Currently there is no straightforward way to build server-only.
  Even if you mark only the server target (and the plugins), sclang is pulled in as well. So a bit of patience is required. You can turn off Qt
  (`-DSC_QT=OFF`) to save on the dependencies. To make it easier to assemble a
  server bundle, the custom target "install_server_only" is provided. It can be used in place of the comprehensive "install". The custom target will copy scsynth's target folder and run fixup_bundle on scsynth to pull in required libraries.

  *Note VS*: As this is not a proper install, the MS runtime libraries
  (msvcp120.dll and msvcr120.dll) are not copied. If required, get them from a full SC install, or the VS/VC/redist folder.
...
batch-files are a series of commands in their line each. You can run
a batch-file automatically upon opening a command-line window, bypassing the
...
If the batch file contains these lines:
    ...
    echo off
    echo Setting up the environment for Qt usage...
...
Another option is batch-files that construct CMake arguments and execute the
build. If for some reason you need to enter many of those and/or can't keep the
build folder with the cache file, this can come in handy. Here is an example that
