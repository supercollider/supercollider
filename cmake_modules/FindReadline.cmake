if(NOT READLINE_INCLUDE_DIR OR NOT READLINE_LIBRARY)
    if(APPLE)
        ...
        find_path(
            ...
            HINTS
                "${CMAKE_SOURCE_DIR}/../${CMAKE_LIBRARY_ARCHITECTURE}/readline/include" # vcpkg
...
if(
    READLINE_INCLUDE_DIR
    AND EXISTS "${READLINE_INCLUDE_DIR}/readline/readline.h"
)
    file(
    ...
    string(
        REGEX REPLACE "^.*RL_VERSION_MAJOR.*([0-9]+).*$"
        "\\1"
        READLINE_MAJOR_VERSION
        "${READLINE_MAJOR_VERSION}"
    )
    string(
        REGEX REPLACE "^.*RL_VERSION_MINOR.*([0-9]+).*$"
        "\\1"
        READLINE_MINOR_VERSION
        "${READLINE_MINOR_VERSION}"
    )
    ...
    if(READLINE_MAJOR_VERSION)
        set(READLINE_VERSION
            "${READLINE_MAJOR_VERSION}.${READLINE_MINOR_VERSION}"
        )
    else()
...
if(READLINE_FOUND)
    if(Readline_FIND_VERSION VERSION_GREATER READLINE_VERSION)
        message(
            STATUS
            "Found version ${READLINE_VERSION} of GNU Readline at ${READLINE_LIBRARY}, but version ${Readline_FIND_VERSION} required"
        )
        ...
        if(NOT READLINE_FIND_QUIETLY)
            message(
                STATUS
                "Found GNU Readline version ${READLINE_VERSION}: ${READLINE_LIBRARY}"
            )
        endif()
    endif()
else()
