...
if(BELA_CFLAGS AND BELA_CXXFLAGS AND BELA_LDFLAGS)
    ...
    if(BELA_CONFIG)
        execute_process(
            COMMAND ${BELA_CONFIG} --defines
            OUTPUT_VARIABLE BELA_DEFINITIONS
        )
        string(STRIP "${BELA_DEFINITIONS}" BELA_DEFINITIONS)
        execute_process(
            COMMAND ${BELA_CONFIG} --includes
            OUTPUT_VARIABLE BELA_INCLUDE_DIRS
        )
        string(STRIP "${BELA_INCLUDE_DIRS}" BELA_INCLUDE_DIRS)
        execute_process(
            COMMAND ${BELA_CONFIG} --libraries
            OUTPUT_VARIABLE BELA_LIBRARIES
        )
        string(STRIP "${BELA_LIBRARIES}" BELA_LIBRARIES)
        execute_process(
            COMMAND ${BELA_CONFIG} --cflags
            OUTPUT_VARIABLE BELA_C_FLAGS
        )
        string(STRIP "${BELA_C_FLAGS}" BELA_C_FLAGS)
        execute_process(
            COMMAND ${BELA_CONFIG} --cxxflags
            OUTPUT_VARIABLE BELA_CXX_FLAGS
        )
    ...
    if(BELA_FOUND)
        if(NOT BELA_FIND_QUIETLY)
            execute_process(
                COMMAND ${BELA_CONFIG} --prefix
                OUTPUT_VARIABLE BELA_PREFIX
            )
