...
foreach(${ICU_PRIVATE_VAR_NS}_COMPONENT ${${ICU_PRIVATE_VAR_NS}_COMPONENTS})
    string(
        TOUPPER "${${ICU_PRIVATE_VAR_NS}_COMPONENT}"
...
if(${ICU_PUBLIC_VAR_NS}_INCLUDE_DIRS)
    ...
    elseif(
        ...
        string(
            REGEX REPLACE ".*# *define *U_ICU_VERSION_MINOR_NUM *([0-9]+).*"
            "\\1"
    ...
    elseif(
        ...
        if(${ICU_PRIVATE_VAR_NS}_FULL_VERSION MATCHES "^([0-9]+)\\.([0-9]+)$")
            ...
            set(${ICU_PUBLIC_VAR_NS}_PATCH_VERSION "0")
        elseif(
            ${ICU_PRIVATE_VAR_NS}_FULL_VERSION
                MATCHES
                "^([0-9]+)\\.([0-9]+)\\.([0-9]+)"
        )
            set(${ICU_PUBLIC_VAR_NS}_MAJOR_VERSION "${CMAKE_MATCH_1}")
    ...
    if(PKG_CONFIG_FOUND)
        ...
        foreach(
            ...
            pkg_check_modules(
                PC_ICU_PRIVATE_VAR_NS
                "icu-${${ICU_PRIVATE_VAR_NS}_COMPONENT}"
                QUIET
            )
    ...
    foreach(
        ...
        foreach(
            ...
            list(
                APPEND ${ICU_PRIVATE_VAR_NS}_POSSIBLE_RELEASE_NAMES
                "${${ICU_PRIVATE_VAR_NS}_BASE_NAME}"
            )
            list(
        ...
        if(
            ...
            elseif(
                NOT ${ICU_PRIVATE_VAR_NS}_LIB_DEBUG_${${ICU_PRIVATE_VAR_NS}_COMPONENT}
            ...
            list(
                APPEND ${ICU_PUBLIC_VAR_NS}_LIBRARIES
    ...
    find_program(
        ${ICU_PUBLIC_VAR_NS}_CONFIG_EXECUTABLE
        icu-config
        HINTS ${${ICU_PRIVATE_VAR_NS}_ROOT}
    )
    ...
    if(
        ${ICU_PUBLIC_VAR_NS}_FIND_REQUIRED
