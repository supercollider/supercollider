...
#ifndef SC_PAUTILS_HPP
#define SC_PAUTILS_HPP
...
static PaDeviceIndex GetPaDeviceFromName(const char* device, IOType ioType) {
...
static PaStreamParameters MakePaStreamParameters(int device, int channelCount, double suggestedLatency) {
...
static PaError CheckDeviceSampleRateOrGetDefault(int* device, double sampleRate, int maxChannels, int defaultDevice,
                                                 const char* deviceType, SupportCheck isSupportedFunc) {
...
static void TryMatchDeviceSameAPI(int* matchingDevice, const int* knownDevice, IOType matchingDeviceType) {
...
static PaError TryGetDefaultDevices(int* inDevice, int* outDevice, int numIns, int numOuts, double sampleRate) {
    if (numIns && !numOuts) {
