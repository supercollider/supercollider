SimpleNumber : Number {
	...
	* { arg aNumber, adverb; _Mul; ^aNumber.performBinaryOpOnSimpleNumber('*', this, adverb) }
	...
	asTimeString { arg precision = 0.001, maxDays = 365;
		var decimal, days, hours, minutes, seconds, mseconds;
		decimal = this.asInteger;
		days = decimal.div(86400).min(maxDays).asString.padLeft(3, "0");
		hours = (decimal.div(3600) % 24).asString.padLeft(2, "0");
		minutes = (decimal.div(60) % 60).asString.padLeft(2, "0");
		seconds = (decimal % 60).asString.padLeft(2, "0");
		mseconds = (this.frac / precision).round(precision).asInteger.asString.padLeft(3, "0");
		^format("%:%:%:%:%", days, hours, minutes, seconds, mseconds) 
	}
