...
if [[ -n "$1" && "$1" == "--qt=true" ]]; then
	sudo apt-get install --yes libgl1-mesa-dev qt59base qt59location qt59declarative qt59tools qt59webchannel qt59xmlpatterns qt59svg qt59websockets
fi
if [[ -n "$2" && "$2" == "--webengine=true" ]]; then
    sudo apt-get install --yes qt59webengine
fi
