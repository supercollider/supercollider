...
template <typename CRTP> struct NamedByte {
    ...
    std::tuple<const char*, int> asTuple() const { return { CRTP::name, asInt() }; }
};
...
template <typename Enum_T, typename CRTP> struct OperandEnumWrapper {
    ...
    // constexpr static CRTP fromRaw(Byte b) { return { Opcode::details::to_enum<Enum_T>(b) }; }

    std::tuple<const char*, const char*> asTuple() const { return { CRTP::name, asName(value) }; }
};
}
...
template <unsigned int TOTAL, unsigned int PART> struct UnsignedInt {
    ...
    constexpr static UnsignedInt<TOTAL, PART> fromFull(size_t i) {
        assert(i < (1ULL << TOTAL));
    ...
    constexpr static UnsignedInt<TOTAL, PART> fromFull(int i) {
        assert(i >= 0);
        return UnsignedInt::fromFull(static_cast<size_t>(i));
    }
    ...
    std::tuple<const char*, int, int, int> asTuple() const {
        if constexpr (TOTAL == 8) {
            return { "Uint", TOTAL, PART, asInt() };
        } else {
            return { "Uint", TOTAL, PART, asIntPart() };
        }
    }
};
...
template <unsigned int TOTAL, unsigned int PART> struct Int {
    ...
    std::tuple<const char*, int, int, int> asTuple() const {
        if constexpr (TOTAL == 8) {
            return { "Int", TOTAL, PART, asInt() };
        } else {
            return { "Int", TOTAL, PART, asIntPart() };
        }
    }
};
