...
start:
	...
	if (c == 0)   { r = 0; goto leave; }
		...
		pushls(&brackets, (ptr_sized_int)c);
		if (c == OPENCURLY) {
			pushls(&closedFuncCharNo, (ptr_sized_int)(linestarts[lineno] + charno - 1));
		}
		r = c;
		goto leave;
	}
		...
		if (!emptyls(&brackets)) {
			if ((d = popls(&brackets)) != (ptr_sized_int) OPENSQUAR) {
				fatal();
		...
		if ((c = input()) == OPENCURLY) {
			pushls(&brackets, (ptr_sized_int)OPENCURLY);
			pushls(&closedFuncCharNo, (ptr_sized_int)(linestarts[lineno] + charno - 2));
			r = BEGINCLOSEDFUNC;
		} else {
			unput(c);
			r = '#';
...
start:
	...
	if (c == 0) goto leave;
		...
		if (!emptyls(&brackets)) {
			if ((d = popls(&brackets)) != (ptr_sized_int) OPENPAREN) {
				fatal();
				post("opening bracket was a '%c', but found a '%c'\n",d,c);
				goto error1;
			}
