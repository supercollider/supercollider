...
template <class T>
inline std::pair<T, std::vector<int>> solve(const std::vector<std::vector<T>>& A) {
    ...
    if (n <= m) a = A;
        ...
        for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j) a[j][i] = A[i][j];
    ...
    for (int j = 1; j <= m; ++j) if (p[j]) assign_rows[p[j] - 1] = j - 1;
    T min_cost = -v[0];

    if (!transposed) return {min_cost, assign_rows};
    // map back if original had rows>cols
    std::vector<int> assign_orig(m, -1);
    for (int i = 0; i < n; ++i) if (assign_rows[i] != -1) {
        int r = assign_rows[i]; assign_orig[r] = i;
    }
