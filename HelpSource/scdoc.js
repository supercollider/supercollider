...
function addInheritedMethods() {
    if (!/\/Classes\/[^\/]+/.test(window.location.pathname))
        return; // skip this if not a class doc
    ...
    for (var i = 0; i < sups.length; i++) {
        ...
        for (var j = 0; j < mets.length; j++) {
            ...
            if (doc.methods.indexOf("_" + m.slice(1)) < 0) { // ignore methods already documented in this helpfile
                ...
                li.innerHTML = "<a href='" + helpRoot + "/" + s.path + ".html#" + encodeURIComponent(m.slice(1)) + "'>"
                    + m.slice(2) + "</a>";
                if (m[1] == "*") {
                    d[0].appendChild(li);
                } else if (m[1] == "-") {
        ...
        for (var j = 0; j < 2; j++) {
            ...
            if (count) {
                ...
                x.innerHTML =
                    count + " methods from <a href='" + helpRoot + "/" + s.path + ".html'>" + s.title + "</a> ";
...
function set_up_toc() {
    var toc_container = $("<div>", { id : "toc-container" }).insertBefore($("#menubar").children().first());
...
function fixTOC() {
    ...
    create_menubar_item("", helpRoot + "/Help.html", function(a, li) {
        a.addClass("home");
        $('<span>', { text : "SuperCollider" }).appendTo(a);
    });
    ...
    create_menubar_item("Indexes \u25bc", "#", function(a, li) {
        ...
        nav_items.forEach(function(item) {
            $("<a>", { text : item, href : helpRoot + "/Overviews/" + item + ".html" }).appendTo(indexes_menu);
        });
