...
function updateTree() {
    ...
    while (1) {
        ...
        for (var i = 0; i < tree.entries.length; i++) {
            ...
            res += x[0].toUpperCase() + "</span><a href='" + docLink + "'>" + v.title
                + "</a></div><div class='result_summary'>" + v.summary + "</div>";
        }
...
function buildCategoryTree() {
    ...
    document.getElementById("total_count").innerHTML =
        countTree({ subcats : tree, entries : [], count : 0 }) + " documents";
}
...
window.onload =
    ...
    window.onhashchange = function() { GotoPath(unescape(window.location.hash.slice(1))); }
