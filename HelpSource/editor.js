const init =
    () => {
        ...
        CodeMirror.defineSimpleMode('scd', {
            start : [
                ...
                {
                    regex :
                        /^(?:false|inf|nil|true|thisFunction|thisFunctionDef|thisMethod|thisProcess|thisThread|currentEnvironment|topEnvironment)\b/,
                    token : 'built-in'
                },
    ...
    textarea.editor.on('dblclick', editor => {
        ...
        let parenMatch = editor.getLine(cursor.line).slice(cursor.ch - 1, cursor.ch).match(/[()]/)
        if (parenMatch) {
...
const selectRegion =
    (options = {
        ...
        if (!textarea) return let editor = textarea.editor
        ...
        const findLeftParen =
            cursor => {
                ...
                return cursorLeft
                let ch = editor.getLine(cursorLeft.line).slice(cursorLeft.ch, cursorLeft.ch + 1)
        ...
        const findRightParen =
            cursor => {
                ...
                return cursorRight
                let ch = editor.getLine(cursorRight.line).slice(cursorRight.ch - 1, cursorRight.ch)
                if (ch === '(')
        ...
        let parenPairs = [] let leftCursor = findLeftParen(cursor)
...
const selectLine =
    (options = {
        ...
        if (editor.somethingSelected()) {
            ...
            from = { line : cursor.line, ch : 0 } to = { line : cursor.line, ch : editor.getLine(cursor.line).length }
        }
