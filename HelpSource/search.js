...
function debug(str) { document.getElementById("js_error").innerHTML += str + "<br>" }
...
function onLoad() {
    ...
    var x = [
        "check_extensions", "check_methods", "ignore_case", "check_word", "check_prefix", "check_suffix", "check_title",
        "check_summary", "check_categories"
    ];
    ...
    document.getElementById("check_regexp").onchange =
        function(e) {
            ...
            for (var mname in methods) {
                ...
                if (re.test(mname)) {
                    ...
                    for (var i = 0; i < m.length; i++) {
                        ...
                        if (!doc)
                            continue;
                        if (doc.installed == "extension" && !include_ext_classes)
                            continue;
                        if (filter_val != "all" && doc.path.split("/")[0].toLowerCase() != filter_val)
                            continue c.push(m[i]);
                    }
                    if (c.length > 0) {
                        mets.push([ mname, c ]);
